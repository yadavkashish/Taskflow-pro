...
import { ImpactAnalysisChanges, ImpactAnalysisResponse, Task, TaskCreateInput } from '../types';
...
interface TaskModalProps {
  ...
  onPreview: (taskId: number, changes: ImpactAnalysisChanges) => Promise<ImpactAnalysisResponse>;
}
...
const deltaLabel = (value: number | null) => value === null ? 'Unavailable' : value > 0 ? `+${value} days delay` : value < 0 ? `${value} days earlier` : 'No change';

const TaskModal: React.FC<TaskModalProps> = ({ isOpen, onClose, task, onSave, onPreview }) => {
  ...
  const [preview, setPreview] = useState<ImpactAnalysisResponse | null>(null);
  ...
  const handlePreview = async () => {
    ...
    setError(null);
    try {
      setPreview(await onPreview(task.id, changes));
    } catch {
      setError('Unable to calculate impact. The task has not been changed.');
    } finally {
      setPreviewing(false);
    }
  };
